...
execute as @e[tag=projectiles] at @s run damage @e[distance=..1.7,tag=!ms_beam_safe,type=!minecraft:arrow,limit=1] 3 nincodedo:sword_beam by @p[predicate=nincodedo:is/entity/inventory/have_master_sword] from @p[predicate=nincodedo:is/entity/inventory/have_master_sword]
